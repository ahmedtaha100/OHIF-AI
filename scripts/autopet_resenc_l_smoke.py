#!/usr/bin/env python
"""Run a bounded AutoPET III ResEnc-L smoke inference and score it."""
...
def main() -> int:
    ...
    try:
        ...
        report["inputs"] = [
            ...
            for path in sorted(args.input_dir.glob("*.nii.gz"))
        ]
        report["label"] = image_fingerprint(args.label)
        ...
        prediction_path = args.output_dir / f"{args.input_dir.glob('*_0000.nii.gz').__iter__().__next__().name[:-12]}.nii.gz"
        report["prediction"] = image_fingerprint(prediction_path)
        report["metrics"] = score_binary(prediction_path, args.label)
        report["status"] = "PASS"
        exit_code = 0
        ...
        print(json.dumps({k: report.get(k) for k in ("status", "total_seconds", "inference_seconds", "metrics", "error")}, indent=2))
    return exit_code
